...
i_sequence = "typeIIIA_cas10_whole_length_adjusted5.csv"
...
pos_kde = density(spacers %>% filter(mapped_strand==1) %>% pull(distance),adjust = 1, kernel = c("gaussian"), window = kernel, n=512,from=-12000,to=12000,bw=50)
pos_kde_frame <- data.frame(pos_kde$x,pos_kde$y)
neg_kde = density(spacers %>% filter(mapped_strand==-1) %>% pull(distance), adjust = 1, kernel = c("gaussian"), window = kernel, n=512,from=-12000,to=12000,bw=50)
...
expected_kde <- density(expected_one, adjust = 1, kernel = c("gaussian"), window = kernel, n=512,from=-12000,to=12000, bw=50)
expected_kde_frame <- data.frame(expected_kde$x,expected_kde$y)
expected_minus_one_kde <- density(expected_minus_one,adjust = 1, kernel = c("gaussian"), window = kernel, n=512,from=-12000,to=12000, bw=50) 
...
spacers <- spacers %>% filter(distance < 12000) %>% filter(distance > -12000)
...
my_out <- paste(i_sequence,"spacer_distribution_plotd_whole_12000_8_9_2024_a_long.png",sep="_")
