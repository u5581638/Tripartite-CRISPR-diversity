...
setwd("D:/spacer_expansion/final_overlay_eric_h_files_29_4_2024/")
i_sequence = "typeVU3.fasta_whole_h.csv"
...
pos_kde = density(spacers %>% filter(mapped_strand==1) %>% pull(distance),adjust = 1, kernel = c("gaussian"), window = kernel, n=2048,from=-50000,to=50000,bw=500)
pos_kde_frame <- data.frame(pos_kde$x,pos_kde$y)
neg_kde = density(spacers %>% filter(mapped_strand==-1) %>% pull(distance), adjust = 1, kernel = c("gaussian"), window = kernel, n=2048,from=-50000,to=50000,bw=500)
...
expected_kde <- density(expected_one, adjust = 1, kernel = c("gaussian"), window = kernel, n=2048,from=-50000,to=50000, bw=500)
expected_kde_frame <- data.frame(expected_kde$x,expected_kde$y)
expected_minus_one_kde <- density(expected_minus_one,adjust = 1, kernel = c("gaussian"), window = kernel, n=2048,from=-50000,to=50000, bw=500) 
...
spacers <- spacers %>% filter(distance < 50000) %>% filter(distance > -50000)
...
my_out <- paste(i_sequence,"spacer_distribution_plotd_whole_50000_8_9_2024a.png",sep="_")
