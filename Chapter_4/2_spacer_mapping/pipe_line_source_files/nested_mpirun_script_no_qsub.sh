...
then 
	python3 /g/data/va71/crispr_pipeline_annotation/pipe_line_source_files/script_constructor_all_calls.py sequence_header.txt nucleotide_database_headers.txt $block_directory $perc_identity $db_dir $spacer_result_dir $query_cover $formatting  #creates the file: single_parallelised_commands.sh
else 
	...
	python3 /g/data/va71/crispr_pipeline_annotation/pipe_line_source_files/script_constructor_all_calls.py sequence_header.txt nucleotide_database_headers.txt $block_directory $perc_identity $db_dir $spacer_result_dir $query_cover $formatting
fi 


mkdir ${db_dir}${spacer_result_dir}
parallel -P $cores -a ${db_dir}parallelised_commands.sh
