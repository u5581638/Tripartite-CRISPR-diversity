# script to color a phylogenetic tree in the same colors as the clusters in casIIIA partitioned network
...
setwd("D:/host_phage_component_partition_reconciliation/subtypes/casIIIA/")
partition_table_color <- read.csv("casIIIA_host_only_partition.csv_top20.csv_w_color.csv") # read in a table which was the color row added. Need to use python to merge.
# can I use regex/sed 
my_tree <- read.tree("typeIIIA_cas10.fasta_all_hits.csv_genomes.fasta_aa_raw.fasta_rep_aa_clustered.fa_aln.fasta.treefile")
...
ggsave(width=16,height=8,"casIIIA_all_tree_top20.png")
